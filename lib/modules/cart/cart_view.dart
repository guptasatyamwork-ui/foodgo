...
class CartView extends GetView<CartController> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        leading: IconButton(icon: const Icon(Icons.arrow_back_ios_new_rounded, size: 20), onPressed: () => Get.back()),
        title: Obx(() => Text('Cart (${controller.cartService.itemCount})')),
        actions: [
          TextButton(onPressed: controller.clearCart,
            child: const Text('Clear All', style: TextStyle(color: AppColors.primary, fontWeight: FontWeight.w600, fontSize: 13))),
        ],
      ),
      body: Obx(() {
  ...
  Widget _emptyCart() {
    return Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      const Text('Add some delicious food!', style: TextStyle(color: AppColors.textSecondary, fontSize: 13)),
      ...
      child: Container(
        ...
        child: Row(children: [
          ClipRRect(borderRadius: BorderRadius.circular(12),
            child: CachedNetworkImage(imageUrl: item.food.imageUrl, width: 76, height: 76, fit: BoxFit.cover,
              placeholder: (_, __) => Container(width: 76, height: 76, color: const Color(0xFFF3F4F6)))),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(item.food.name, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 14, color: AppColors.textPrimary),
  ...
  Widget _row(String l, String v, {bool bold = false}) => Row(
    ...
    children: [
      Text(l, style: TextStyle(color: bold ? AppColors.textPrimary : AppColors.textSecondary,
        fontSize: bold ? 16 : 13, fontWeight: bold ? FontWeight.w700 : FontWeight.w400)),
      Text(v, style: TextStyle(color: bold ? AppColors.primary : AppColors.textPrimary,
        fontSize: bold ? 18 : 13, fontWeight: bold ? FontWeight.w800 : FontWeight.w600)),
    ]);
